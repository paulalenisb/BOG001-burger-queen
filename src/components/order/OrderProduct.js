    ...
    order.map((product, index)=>(
      <div className='Order-product' key={index}>    
        ...
        <div className='Order-quantity'>
          <button className='Btn-product' aria-label='Restar producto'onClick={()=>{ quantity - 1 === 0 ? setOrder(order.filter((_,idx) => idx !== index )): setQuantity(quantity-1)}}>
        ...
        <div className='Order-price'>
          ...
          <button className='Btn-product' aria-label='Eliminar producto' >
            <i className='fas fa-trash'onClick ={ () => setOrder(order.filter((_,idx) => idx !== index ))} ></i>
          </button>
        </div>
      </div>
      ))
